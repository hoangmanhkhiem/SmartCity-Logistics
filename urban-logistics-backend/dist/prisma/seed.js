...
async function main() {
    ...
    const users = await Promise.all([
        ...
        ...Array.from({ length: 10 }, (_, i) => prisma.user.create({ data: { email: `user${i + 11}@test.com`, password: hashedPassword, name: `User ${i + 11}`, phone: `090100001${i + 1}` } })),
    ]);
    ...
    const organizations = await Promise.all([
        ...
        ...Array.from({ length: 12 }, (_, i) => prisma.organization.create({ data: { name: `Công ty vận tải ${String.fromCharCode(65 + i)}`, type: i % 2 === 0 ? 'delivery' : 'logistics', business: 'Vận tải', address: `Số ${i + 10} Đường ${i + 1}, Hà Nội`, email: `company${i + 1}@test.vn` } })),
    ]);
    ...
    const carriers = await Promise.all([
        ...
        ...Array.from({ length: 15 }, (_, i) => prisma.carrier.create({ data: { organizationId: organizations[Math.min(i % 8, 7)].id, name: `Carrier ${i + 6}`, scale: i % 3 === 0 ? 'Lớn' : 'Nhỏ', vehicleCount: 50 + i * 10, warehouseCount: 5 + i } })),
    ]);
    ...
    const zones = await Promise.all([
        ...
        ...Array.from({ length: 12 }, (_, i) => prisma.zone.create({ data: { name: `Quận ${i + 7}`, type: 'district', description: `Quận ${i + 7} Hà Nội` } })),
    ]);
    ...
    const facilities = await Promise.all([
        ...
        ...Array.from({ length: 14 }, (_, i) => prisma.facility.create({
            data: {
    ...
    const vehicles = await Promise.all(Array.from({ length: 20 }, (_, i) => prisma.vehicle.create({
        data: {
    ...
    const roads = await Promise.all(Array.from({ length: 20 }, (_, i) => prisma.roadSegment.create({
        data: {
            zoneId: zones[i % 8].id,
    ...
    const routes = await Promise.all(Array.from({ length: 20 }, (_, i) => prisma.route.create({
        data: {
    ...
    const orders = await Promise.all(Array.from({ length: 20 }, (_, i) => prisma.order.create({
        data: {
    ...
    const shipments = await Promise.all(orders.map((order, i) => prisma.shipment.create({
        data: {
            orderId: order.id,
    ...
    const legs = await Promise.all(shipments.slice(0, 15).map((shipment, i) => prisma.leg.create({
        data: {
